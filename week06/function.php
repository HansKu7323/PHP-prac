<?php
	...
	$str = "abcdefg";
	$get = substr($str, 3, 3);
		//輸出 abcdefg 字串取得從index第3個取3碼：def
	echo $str . " 字串取得從第0個取3碼：「" . $get . "」<br>";

	$get2 = substr($str, 2);
	//輸出 abcdefg 字串取得從index第2個取到最後面一個字：cdefg
	echo $get2 , "<br>" ;

	$get3 = substr($str, -4);
	//輸出 abcdefg 字串取得從index後面倒數過來第4個取到最後面一個字：defg
	echo $get3;
	...
	//輸出 IE 瀏覽器要被微軟拋棄了 字串取得從第10個取3字：拋棄了，空白也算進字數

	$get2 = mb_substr($str, 3);
	echo $str . " 字串取得從第3個取到最後一個字：「" . $get2 . "」<br>";

	$get3 = mb_substr($str, -6);
	echo $str . " 字串取得從倒數第6個取到最後一個字：「" . $get3 . "」<br>";

	echo "<hr>";
	//使用 str_replace() 把字串中的文字置換
	echo '<p>str_replace() 方法</p>';
	$str = "IE 瀏覽器要被微軟拋棄了，回顧一下它被網民嫌棄的一生吧";
	$get = str_replace("IE 瀏覽器", "討厭的IE，難用死了", $str);
	echo $str . "<br>" . $get . "<br>";
	//輸出 討厭的IE，難用死了要被微軟拋棄了，回顧一下它被網民嫌棄的一生吧
	...
	echo "<br>";
	echo strpos("迫降台南F/A-18戰機　隸屬美軍陸戰隊","A"); //輸出 14
	
	echo "<hr>";
	//使用 mb_strpos() 判別某字在該字串中的那個字數位置
	echo '<p>mb_strpos() 方法</p>';
	echo mb_strpos("迫降台南F/A-18戰機　隸屬美軍陸戰隊","戰機"); //輸出 10
	echo '<br>';
	echo mb_strpos("迫降台南F/A-18戰機　隸屬美軍陸戰隊","/"); //輸出 5
